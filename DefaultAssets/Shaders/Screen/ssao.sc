...
void main() {
    ...
    float outerNormalEdge = max(0.0, horizontalDelta) + max(0.0, verticalDelta);
    ...
    float depthRidge = ((dL + dR + dT + dB) * 0.25) - depthC;
    ...
    gl_FragColor = vec4(vec3_splat(1.0 - clamp(edge, 0.0, 1.0)), 1.0);
}