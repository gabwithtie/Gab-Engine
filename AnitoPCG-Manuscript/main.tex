...
Anito Builder primarily focused on procedurally generating the the Saint La Salle Hall at De La Salle University, Philippines. It accomplished this by using neo-classical architecture as a reference, which is a type of architecture from the 18th century that emphasizes symmetry and grandeur. 

However, some buildings and structures cannot be generated due to copyright issues or other legal reasons, and the researchers were not able to get official building schematics, making all generations inspired by Lasallian architecture instead of a one-to-one match of the real-life buildings/locations. This includes murals, sculptures, logos, and artworks that can be found within or on the buildings. Similarly, floor plans, internal designs, and administrative rooms or buildings cannot be obtained or recreated without permission.
...
For PCG content, the 3D Asset workflows begin with a creative direction. Artists and the software developer discuss what art style the assets should follow, followed by gathering references, making the mesh models, and then texturing the 3D assets. The chosen workflow has the following steps:
...
\begin{enumerate}
    ...
    \item UV Unwrapping: To facilitate subsequent steps, the UV coordinates of each mesh was unwrapped in such a way that avoided overlapping and stretching.
    \item Texturing: Once the mesh of the 3D asset is done, the mesh must be textured. To ensure procedurality, simple tile-able textures was created for general use on all meshes.
...
An application named "Anito Builder" was be created for the project. It is standalone 3D editor application that aims to allow users to intuitively use the created PCG method to generate custom scenes.
...
The final collection of models can be classified into 7 categories:
    ...
    \subsubsection{1st Floor Wall Segments}
...
\end{figure}
    ...
    \subsubsection{2nd Floor Wall Segments}
...
\end{figure}
    ...
    \subsubsection{3rd Floor Wall Segments}
...
\end{figure}
    ...
    \subsubsection{Rooftop Wall Segments}
...
\end{figure}
    ...
    \subsubsection{2-Segment Wall Segments}
...
\end{figure}
    ...
    \subsubsection{3-Segment Wall Segments}
...
\subsection{3D Asset Collection}
...
\subsection{3D Asset development}
