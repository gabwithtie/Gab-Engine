...
\subsection{TODO: Find more studies for engines}
...
\subsection{Concepts in Software Architecture}
...
Using a low-level language is essential to creating game engines or libraries. A low-level language is one that is close to assembly language (i.e., the language that computers can understand natively) (Ayepeku, F. O. n.d.). This is important for libraries because creating high-performance wrappers will be easier.
...
The main concept of the 3D editor is based on editable blocks similar to what one can find on Unity's Probuilder or Blender's face edit mode. Anito Builder can spawn cubes in which all 4 faces of the cube are selectable and movable in the x-plane, while the cube's height can be controlled by dragging the roof up or down.
...
Given a set of inputs, the algorithm will generate a list of objects containing the mesh type and its transformation. This list can be used by the external environment to render the result. The algorithm is a simple loop that iterates over a plane that is defined by the input parameters.

\subsubsection{\textbf{Structure of the 3D editor}}

The 3D editor itself is created using well-known libraries. The editor follows the hierarchical component system that is used by Godot, while the user experience and interfaces are heavily inspired by Unity.
...
\begin{figure}[H]
	...
	\begin{tikzpicture}[
		...
		container/.style={draw, rectangle, dashed, inner sep=0.5cm, rounded corners, fill=gray!5},
		% Style for boxes
		base/.style={draw, rectangle, minimum width=3cm, minimum height=0.8cm, text centered, fill=white, drop shadow},
		header/.style={base, fill=blue!20, font=\bfseries\small\sffamily},
		manager/.style={base, fill=yellow!10, dashed},
		% Style for arrows
		arrow/.style={thick, -{Stealth[scale=1.2]}}
		]
		
		% --- OBJECT SYSTEM SECTION ---
		\node[header] (root) {Root Object};
		\node[manager, below=of root] (managers) {Managers};
		
		% Level 2 Objects
		\node[base, below=1.5cm of managers] (renderObj) {Render Object};
		\node[base, left=of renderObj] (cameraObj) {Camera Object};
		\node[base, right=of renderObj] (anitoObj) {Anito Builder Object};
		
		% Container for Object System
		\begin{scope}[on background layer]
			\node[container, fit=(root) (managers) (cameraObj) (anitoObj), label={[shift={(0,-0.5)}]above:\textbf{Object System}}] (objSys) {};
		\end{scope}
		
		% --- OTHER SECTIONS ---
		\node[header, below=2cm of anitoObj] (pcgMethod) {Anito Builder PCG Method};
		
		% Graphics System Group
		\node[header, below=2cm of renderObj] (graphicsHead) {Graphics System};
		\node[base, below=0.5cm of graphicsHead] (bgfx) {BGFX};
		\node[base, below=0.3cm of bgfx] (imgui) {IMGUI};
		\node[base, below=0.3cm of imgui] (forward) {Forward Rendering};
		\node[base, below=0.3cm of forward] (instanced) {Instanced Rendering};
		
		% Container for Graphics System
		\begin{scope}[on background layer]
			\node[container, fit=(graphicsHead) (instanced), fill=blue!5] (gfxSys) {};
		\end{scope}
		
		% --- DRAWING ARROWS ---
		\draw[arrow] (root) -- (managers);
		\draw[arrow] (managers.south) -- ++(0,-0.8) -| (cameraObj.north);
		\draw[arrow] (managers.south) -- ++(0,-0.8) -| (renderObj.north);
		\draw[arrow] (managers.south) -- ++(0,-0.8) -| (anitoObj.north);
		
		\draw[arrow] (anitoObj) -- (pcgMethod);
		\draw[arrow] (renderObj) -- (gfxSys.north);
		
	\end{tikzpicture}
	\caption{Anito Engine Architecture: Object and Graphics Systems Hierarchy}
\end{figure}

TODO: Expound on engine architecture and optimizations done.

\subsubsection{\textbf{Viewing and interacting with the result in the 3D editor}}

The 3D editor is complete with camera controls, object selection/movement, inspecting, and saving/serializing. These features can be utilized by users to create and explore the capabilities of Anito Builder.

The 3D editor has the ability to create an exported 3D file for use in other applications. This is helpful if the user wants to evaluate the output of Anito Builder in an environment they are already familiar with.
...
For the unstructured time, they were requested to create and explore the features of the editor without assistance. While for the structured time, they were asked to complete tasks wherein minimal assistance was given. The data gathered from these tests were based on the methods used by Kamińska(2022). These include but are not limited to:
...
To complement this perception-based evaluation of the generated scenes, we also conducted a System Usability Scale (SUS) test to assess the usability of the PCG-based 3D scene editor itself. The SUS is a widely adopted and validated tool in HCI research for quickly gauging overall system usability, and it provides a standardized score that can indicate whether the interface is efficient, intuitive, and accessible to users. This dual-evaluation strategy ensures that the study captures both the user’s visual and architectural impressions of the generated content and the functional usability of the editor that produces it.
